...
const retrieveRegionInfo = (bot, regions) => {
  ...
  const onMessage = (jsonMsg) => {
    if (messageUtils.noRegionFound(messageUtils.parseChatMessage(jsonMsg))) {
      loggingUtils.log('No region could be found with the name: ' + regions[curr]['warp'], bot)
  ...
  const sendNextChat = () => {
    setTimeout(() => bot.chat(`/rg i ${regions[curr]['warp']}`), messageUtils.MESSAGE_DELAY)
  }
  ...
  const isRegionInfo = msg => {
    ...
    if (!msg['json']['extra'][0]['extra'] || msg['json']['extra'][0]['extra'].length <= 2) {
      return false
    }
  ...
  const getRegionInfo = msg => {
    ...
    for (let i = 0; i < regionNodes.length; i++) {
      if (regionNodes[i]['text'] === 'Owners: ') {
        ...
        if (regionNodes[i + 1]['extra'][0]['extra'] === undefined) {
          ...
          regionInfo['mayorNames'] = regionNodes[i + 1]['extra'][0]['extra'].map(o => o['text']).filter(o => o !== ', ')
        }
      }
    }
    ...
    for (let i = 0; i < regionNodes.length; i++) {
      if (regionNodes[i]['text'] === 'Bounds:') {
        const coordinates = messageUtils.parseCoordinatesLine(regionNodes[i + 1]['text'].trim())
